...
\end{align}
\section{Solution}
\textbf{Step1}: To Find the solution using\\
\begin{align*}
\myvec{2 & 3 }\vec{x}&=11 \\ 
\myvec{2 & -4 }\vec{x}&=-24 \\
\\
\end{align*}
To find the solution, check the consistency of equations by using the rank of matrix.\\
\begin{align*}
N=\begin{pmatrix}2 & 3\\2 & -4\\\end{pmatrix}\\
M=\begin{pmatrix}2 & 3 & 11\\2 & -4 & -24\\\end{pmatrix}\\
\end{align*}
since rank of matrix N =2 and M =2, there exists a unique solution which is intersection point of two line equations. \\To Find the solution,use ratio of determinants methods for $x$ and $y$ and form a 2x2 matrix from above equations to get into A$x$=B format.\\
\begin{align*}
...
\Rightarrow m=-1
\end{align}
The solution of all the three equations i.e.
\begin{align*}
\begin{pmatrix}x\\y\\\end{pmatrix}=\begin{pmatrix}-2\\5\\\end{pmatrix}
\end{align*}
can be verified from the plot of vectors as below\\
The vectors of equations are plotted on 2D axes by taking intersecting points on x and y axes respectively.Intersecting points are given in code\\
\begin{lstlisting}
